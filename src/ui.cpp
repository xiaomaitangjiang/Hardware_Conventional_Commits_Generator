...
auto BuildCommitMessage(const UIState& s, const TypeRegistry& types,
                        const ScopeRegistry& scopes) -> CommitMessage
{
  ...
  if (!s.module.empty()) builder.SetModule(s.module);
  if (!s.description.empty()) builder.SetDescription(s.description);
  if (!s.body.empty()) builder.SetBody(s.body);
...
Component MakeBodyInput(UIState& s)
{
  ...
  return Input(&s.body, "enter body (optional, Ctrl+J for newline)",
               std::move(option));
}
...
Components MakeFooterInput(UIState& s)
{
  ...
  for (int i = 0; i < kMaxFooterSlots; ++i) {
    key_toggles.push_back(Toggle(&footer_keys, &s.footer_slots[i].key_index));
    value_inputs.push_back(Input(&s.footer_slots[i].value, "enter value"));
  }

  Components children;
  for (int i = 0; i < kMaxFooterSlots; ++i) {
    // Event routing: both components receive events
    auto event_container =
        Container::Horizontal({key_toggles[i], value_inputs[i]});
...
Component MakeActionBar(UIState& s, ScreenInteractive& screen,
                        const TypeRegistry& types, const ScopeRegistry& scopes)
{
  auto generate_btn = Button("Generate & Copy", [&] {
    s.generated_message =
        FormatCommitMessage(BuildCommitMessage(s, types, scopes));
...
auto RunTUI(TypeRegistry& types, ScopeRegistry& scopes) -> std::string
{
  ...
  std::vector<Component> children = {
      type_selector, scope_selector, module_input, desc_input, body_input,
  };
  ...
  auto main_renderer = Renderer(main_container, [&] {
    ...
    auto counter =
        text(" " + std::to_string(desc_len) + "/72") | color(counter_color);

    Elements rows;

    // Title
    rows.push_back(text(" Hardware Conventional Commits Generator ") | bold |
                   center);
    rows.push_back(separator());

    // Type / Scope
    rows.push_back(
        hbox({text(" Type:        "), type_selector->Render() | flex}));
    rows.push_back(
        hbox({text(" Scope:       "), scope_selector->Render() | flex}));

    // Module
    rows.push_back(
        hbox({text(" Module:      "), module_input->Render() | flex}));

    // Description
    rows.push_back(
        hbox({text(" Description: "), desc_input->Render() | flex, counter}));
    ...
    rows.push_back(body_input->Render() | border |
    ...
    rows.push_back(paragraph(preview) | border | size(HEIGHT, GREATER_THAN, 4));
    ...
    rows.push_back(hbox({
        text(" Tab: nav  |  Enter: toggle  |  ") | dim | center | flex,
        action_bar->Render(),
    }));
